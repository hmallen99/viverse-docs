...
- Reduce visual fidelity - If the app is GPU-bound, i.e. too much time is spent each frame drawing triangles to the canvas, consider lowering the quality of the scene. This can include: lowering the polygon count of meshes, reducing texture sizes, reducing the number of dynamic lights, removing expensive math functions from shaders, and removing post-processing effects like fog.
